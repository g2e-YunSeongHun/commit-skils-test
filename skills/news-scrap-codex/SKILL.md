...
- 국내 기사가 비어 보이면 바로 `0건`으로 확정하지 말고, `필수 도메인 순회`와 `진료 AI/119·구급/문서화 축 보조 검색`을 먼저 수행한다.
- 기본 수집은 국내/해외 기사 원문 중심이다.
- `트리아지`는 진료 AI 축에 포함한다. `병상·환자흐름·전원`과 일반 `소방 AI/로봇/위원회` 기사는 기본 국내 수집에서 제외하고, 사용자가 운영/정책 축을 명시할 때만 fallback으로 검토한다.
