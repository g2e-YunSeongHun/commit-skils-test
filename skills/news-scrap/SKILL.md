...
수집된 기사 중 **가장 중요한 1건**을 선정하여 3장짜리 발표 슬라이드를 생성한다.
...
#### 8-2. NotebookLM에 기사 요약 + 기관/기술 소개 질의

선정된 기사에 대해 NotebookLM에 아래 질의를 실행한다:

```
아래 기사에 대해 한국어로 답변해줘:
기사 제목: {선정된 기사 제목}

1. 기사요약: 이 기사의 핵심 내용을 5~8문장으로 요약해줘. 누가, 무엇을, 왜, 어떻게 했는지 중심으로.

2. 기관소개: 이 기사에 등장하는 주요 기업/연구기관/병원을 각각 소개해줘 (2~3개). 각 기관마다:
   - 기관명
   - 어떤 곳인지 (설립 연도, 소재지, 분야 등)
   - 대표적인 기술/제품/성과
   - 이 기사에서의 역할
   형식: 기관당 3~5문장으로 서술.

3. 기술소개: 이 기사에서 다루는 핵심 기술/시스템/알고리즘을 소개해줘 (1~2개). 각 기술마다:
   - 기술/제품명
   - 무엇을 하는 기술인지 (원리, 작동 방식)
   - 성능 수치 (정확도, AUC, 처리 시간 등 기사에 나온 숫자)
   - 현재 도입/활용 현황
   형식: 기술당 3~5문장으로 서술.

4. 적용아이디어: 이 기사의 기술/연구 결과를 NFA 응급환자 최적 이송 플랫폼(119 구급차-병원 AI 기반 최적 이송)에 적용할 수 있는 아이디어 3개를 제안해줘. 각 아이디어마다:
   - 제목 (한 줄)
   - 구체적 적용 방안 설명 (2~3문장)
   형식: 아이디어당 제목 + 설명으로 서술.
...
답변을 파싱하여 `기사요약`, `기관소개`(배열), `기술소개`(배열), `적용아이디어`(배열)를 추출한다.

#### 8-3. 슬라이드 생성

아래 JSON을 임시 파일로 저장한 후, `scripts/slide.py`를 실행한다:
...
```bash
python {이 스킬의 scripts/slide.py 절대경로} news_output/slide_{해당년도월주차}.pptx {임시JSON파일경로}
```

- 출력 파일: `news_output/slide_{해당년도월주차}.pptx`
- 같은 파일이 있으면 덮어쓴다
...
슬라이드 생성 완료:
- 주목 기사: {선정된 기사 제목}
- 파일: news_output/slide_{해당년도월주차}.pptx
```
