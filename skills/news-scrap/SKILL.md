...
```
## 원문 추출 및 발행일 확인 (필수 — 모든 기사에 적용)

각 기사마다 아래 절차를 수행한다:

### 원문 추출
WebFetch 도구로 `https://r.jina.ai/{기사URL}`을 호출하여 본문 텍스트를 추출한다.
- prompt: "이 웹페이지의 본문 전체를 그대로 반환해줘. 광고, 네비게이션, 관련기사 목록은 제외하고 본문 내용만."
- 추출된 본문을 `원문` 필드에 저장한다.
- `r.jina.ai` 실패 시 → 기존 WebFetch로 직접 URL 접근 → 그래도 실패 시 Playwright MCP (mcp__playwright__playwright_navigate → mcp__playwright__playwright_get_visible_text) 사용

### 발행일 확인
원문 추출 과정에서 발행일도 함께 확인한다:
1. `r.jina.ai` 응답에서 날짜 추출 (대부분 본문 상단에 발행일 포함)
2. 날짜를 찾지 못하면 → WebFetch로 원본 URL 접근하여 발행일 확인 (prompt: "이 기사의 정확한 발행일을 YYYY-MM-DD 형식으로만 답해줘. 찾을 수 없으면 NOT_FOUND.")
3. 그래도 실패 → URL 경로에서 날짜 패턴 추출 (/2026/03/, 2026-03-12 등)
4. URL에도 없으면 → WebSearch 스니펫에서 날짜 추출
5. 위 모든 방법 실패 → 해당 기사 **제외**

발행일이 {시작일}~{종료일} 범위 밖이면 제외한다. "추정" 날짜 금지.
...
> **사전 조건:** `pip install notebooklm-py` 설치 및 `notebooklm login` 인증 완료 상태
...
Step 3에서 중복 제거된 **모든 기사를 하나의 텍스트로 합쳐** `add_text` 1회 호출로 업로드한다.
...
[기사 제목 그대로]
- 기업/기관 소개: 해당 기사에 등장하는 기업이나 기관이 어떤 곳인지 (설립년도, 규모, 주요 사업 등)
- 기술/제품 설명: 어떤 기술이나 제품인지, 어떻게 작동하는지 구체적으로
- 핵심 수치/성과: 정확도, AUC, 비용 절감, 도입률, 처리 시간 등 기사에 나온 모든 구체적 숫자
- 의의/맥락: 이 기사가 응급의료 AI 분야에서 왜 중요한지, 어떤 문제를 해결하는지
- (연구/논문인 경우) 연구 방법: 대상 수, 연구 설계, 사용 모델. 주요 결과 및 한계점
```
...
- `r.jina.ai` 접근 실패 시: WebFetch 직접 접근 → Playwright MCP 순으로 폴백
- `notebooklm-py` 오류 시: 에러 메시지를 출력하고 HTML 대시보드 생성은 정상 진행 (NotebookLM 연동은 선택적)

## Notes

- 이 스킬은 Claude Code의 WebSearch, `r.jina.ai` 원문 추출, `notebooklm-py` 연동으로 동작한다
