...
8. `의료 AI 인공지능 병원 도입 {시작일}~{종료일}` _(와일드카드)_
...
7. `hospital AI clinical workflow automation {시작일}~{종료일}` _(와일드카드)_
...
1. **와일드카드 키워드 필터링:** "응급/emergency"가 포함되지 않은 키워드로 수집된 기사는 응급실, 응급의학, 중환자, 구급, 이송 등과 직접적 관련이 있는 경우에만 포함한다. 순수 일반 의료 AI 기사(예: 피부과 AI, 안과 AI 등)는 제외한다.
2. **blocked_domains:** `["youtube.com", "blog.naver.com", "tistory.com", "brunch.co.kr", "medium.com", "velog.io", "reddit.com"]`
3. **발행일 확인 순서 (필수):**
   ...
   - 위 모든 방법 실패 → 해당 기사 **제외**
4. **날짜 필터링:** 발행일이 해당 주차 범위({시작일}~{종료일}) 내인 기사만 포함. "추정" 날짜 금지.
5. **중복 제거**, **모든 출력은 한국어**
6. **반환 형식:** 아래 필드를 포함하는 JSON 배열로 최종 결과만 반환:
   ```json
   [
     {
       "기관매체": "출처 매체명",
