...
- 재실행 전 `python scripts/reset_week_outputs.py <run_dir>`를 실행한다.
- 모든 중간 파일은 `<work_dir> = <run_dir>/_work`에 둔다.

### 2. Search

- `scripts/build_search_queries.py --output <work_dir>/search_queries.json`로 주차별 검색 힌트를 만든다.
...
- 발행일이 대상 기간 밖이면 제외한다.
- 원문 검증이 어려운 페이월·중복·본문 불충분 기사는 제외한다.
- 제목보다 본문 맥락을 우선한다. 응급실, 응급의료, 중환자실, 의료기관 진료, 의료기기·의료영상 맥락이 본문에서 확인되면 포함 후보로 둔다.

### 4. Write Briefing Markdown

- Codex가 검증된 기사 목록, 한국어 요약, 대표 기사, 심층 리서치, 슬라이드 문안을 `<work_dir>/news_briefing.md`에 작성한다.
...
- 해외 기사 요약이 한국어로 보이지 않으면 렌더링이 실패한다. 이 경우 `news_briefing.md`의 해당 요약을 다시 번역·검토한다.
...
- Pencil MCP slot node 검색 또는 업데이트 실패
- 슬라이드가 4장이 아닌 상태
- PDF export 실패
- `finalize_pencil_slide_pdf.py` 정규화 실패

위 경우 즉시 종료하고, 가능하면 `<work_dir>/pencil_failure.json` 또는 실행 로그에 실패 단계와 원인을 남긴다.
