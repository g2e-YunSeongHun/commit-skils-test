...
- 고정 4장 Pencil 템플릿의 `bind.*` 자리값을 기사별 리서치 내용으로 채워야 할 때

## 핵심 규칙

- 최종 산출물은 `<run_dir>/news_<week_id>.html`과 `<run_dir>/news_slide_<week_id>.pdf` 두 개만 남긴다.
- NotebookLM과 PPTX는 사용하지 않는다.
- Codex는 검색·검증·요약·대표 기사 선정·심층 리서치 결과를 `<work_dir>/news_briefing.md` 하나로 구조화한다.
- HTML과 Pencil slide spec은 모두 `news_briefing.md`에서 생성한다.
...
- Pencil 템플릿의 반복 카드 수는 최대치다. 조사에서 확인된 항목만 채우고, 근거 없는 핵심 포인트·기능·제품·서비스·요약 항목은 만들지 않는다.
...
3. `<work_dir>`를 `<run_dir>/_work`로 둔다.
4. `python scripts/build_search_queries.py --start-date YYYY-MM-DD --end-date YYYY-MM-DD --format json --output <work_dir>/search_queries.json`로 검색 세트를 만든다.
...
8. Codex가 검증된 기사, 한국어 요약, 대표 기사, 심층 리서치, 4장 슬라이드 자리값을 `<work_dir>/news_briefing.md`로 작성한다. 구조는 [references/briefing-md-contract.md](references/briefing-md-contract.md)를 따른다.
9. `python scripts/render_dashboard_from_md.py <work_dir>/news_briefing.md <run_dir>/news_<week_id>.html`로 HTML 대시보드를 만든다.
10. `python scripts/build_pencil_slide_spec_from_md.py <work_dir>/news_briefing.md <work_dir> --week-id <week_id>`로 Pencil `template_bindings` spec을 만든다.
11. `python scripts/prepare_pencil_template.py <work_dir> --spec-json <work_dir>/pencil_slide_spec.json`로 기존 Pencil 템플릿을 주차별 작업 파일로 복사한다.
12. `python scripts/apply_pencil_bindings.py <work_dir>/news_slide_<week_id>.pen <work_dir>/pencil_slide_spec.json --strict`로 `bind.*` text node를 자동 갱신하고 빈 반복 카드를 숨긴다.
13. 복사·갱신된 `<work_dir>/news_slide_<week_id>.pen` 파일을 Pencil에서 열거나 Pencil MCP `open_document(path=...)`로 active editor로 만든다.
14. Pencil MCP `snapshot_layout(problemsOnly=true)`로 clipped element가 없는지 확인한다.
15. Pencil MCP `export_nodes(format="pdf")`로 slide frame 4개를 PDF로 export한다.
16. `python scripts/finalize_pencil_slide_pdf.py <exported_pdf> <run_dir> --spec-json <work_dir>/pencil_slide_spec.json --pen-file <work_dir>/news_slide_<week_id>.pen --work-dir <work_dir>`로 PDF 파일명을 정규화하고 `_work`를 삭제한다.
...
- Pencil 템플릿 binding 계약: [references/pencil-template-contract.md](references/pencil-template-contract.md)
- 출력 파일 계약: [references/output-contracts.md](references/output-contracts.md)

## 포함 스크립트

- `scripts/build_search_queries.py`: 주차별 검색어와 site-pass 세트를 생성한다.
...
- `scripts/build_pencil_slide_spec_from_md.py`: `news_briefing.md`에서 Pencil `template_bindings` spec을 만든다.
- `scripts/prepare_pencil_template.py`: 기존 `.pen` 템플릿을 주차별 작업 파일로 복사한다.
- `scripts/apply_pencil_bindings.py`: 주차별 `.pen` 파일의 `bind.*` text node를 갱신하고 빈 반복 카드를 숨긴다.
...
호환용 helper인 `freeze_verified_articles.py`, `select_featured_article.py`, `render_dashboard.py`, `build_pencil_slide_spec.py`도 남아 있지만 v2 기본 경로에서는 `news_briefing.md`를 원본으로 사용한다.
